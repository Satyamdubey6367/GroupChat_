...
const Chatlist = (props) => {
    ...
    const [messegeTime, setmessegeTime] = useState("")
     console.log(messages," done");
    const apiCall = () => {
        axios.get("http://localhost:3001/massage/getMassage").then((res) => {
            console.log(res.data.massages ,"api call");
    ...
    return (
        <div><div className="flex flex-col flex-grow w-full max-w-xl bg-white shadow-xl rounded-lg overflow-hidden" style={{ height: "100vh", width: "700px" }}>
                ...
                messages.map((mess)=>{
                    return(
                        <div>{mess.userId==messageData.userId ? 
                            ...
                            <div className="flex-shrink-0 h-10 w-10 rounded-full bg-gray-300 text-3xl text-center uppercase">{mess.userEmail[0]}</div>
            ...
            <button style={{ backgroundColor: "green", height: "50px", width: "600px" }} onClick={() => {
                console.log(messageData)
                axios.post("http://localhost:3001/massage/addMassage", messageData).then((res) => {
                    console.log(res);
